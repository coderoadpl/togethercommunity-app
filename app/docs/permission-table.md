...
Closed capability count: 87. Route rows: 189. Exported `Ctx` use-case rows: 163.
...
| api-key | `apps/server/src/internal-app.ts:739` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:741` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1183` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1183` | `(identity.staffRole \|\| identity.memberId)` |
