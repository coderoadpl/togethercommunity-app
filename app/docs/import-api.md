...
- Import scopes cannot be combined with non-import scopes (`enrollment`, `marketing`, `transactional`, `subscriptions:read`, `subscriptions:adopt`, or `report:read`) on the same key. Existing unscoped keys never gain import access.
