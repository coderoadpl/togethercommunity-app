...
Closed capability count: 99. Route rows: 235. Exported `Ctx` use-case rows: 198.
...
| api-key | `apps/server/src/internal-app.ts:856` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:858` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1301` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1301` | `(identity.staffRole \|\| identity.memberId)` |
