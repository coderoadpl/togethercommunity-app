...
## Redirects for links minted elsewhere
...
results. Together keeps a redirect table per workspace and answers it on every
...
with the content is imported as temporary, so a reader who followed the link
during the cutover is never pinned to a stale destination by their own browser
cache. The query string is carried over, so campaign parameters survive the hop.

The lookup normalises the request path before it matches: case is ignored,
repeated slashes collapse, and a trailing slash is dropped. Entries belong to
