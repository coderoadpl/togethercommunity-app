...
try {
  for (const screen of ['lesson', 'start', 'space-feed', 'hosted-legal-document']) {
    for (const viewport of [{ name: 'desktop', width: 1440, height: 900 }, { name: 'mobile', width: 390, height: 844 }, { name: 'mobile-375', width: 375, height: 812 }]) {
      if (screen === 'hosted-legal-document' && viewport.name === 'mobile-375') continue;
      for (const mode of ['light']) {
        ...
        const id = `pages-${title}--${mode}-${viewport.name === 'desktop' ? 'desktop' : 'mobile'}`;
        ...
        try {
          await page.goto(`http://studio.localhost:${address.port}/iframe.html?id=${id}&viewMode=story`, { waitUntil: 'load' });
          if (screen === 'hosted-legal-document') {
            await page.frameLocator('iframe[title="Hosted legal document"]').getByTestId('hosted-legal-document').waitFor({ timeout: 20000 });
          } else {
            const ready = screen === 'lesson' ? 'author-chip-post-js-zmienne-q-r2' : screen === 'start' ? 'start-courses' : 'space-follow-toggle';
            await page.getByTestId(ready).waitFor({ timeout: 20000 });
            await page.waitForFunction(() => document.documentElement.dataset['fixtureReady'] === 'true');
          }
          await settlePage(page);
          if (screen === 'hosted-legal-document') await page.frameLocator('iframe[title="Hosted legal document"]').locator('body').evaluate(async () => { await document.fonts.ready; });
