...
const plan: Scenario[] = [
  ...
  { name: 'checkout', principal: 'anonymous', tenant: 'studio', page: 'checkout', route: '/checkout/product-studio-course-101', courseId: '', lessonId: '', spaceId: '', extra: async (api) => { await api.publicPaymentConfig(); } },
  ...
  { name: 'search', principal: 'student.active@together.dev', tenant: 'studio', page: 'search', route: '/search', courseId: 'course-js', lessonId: '', spaceId: '', extra: async (api) => { await api.studentCourseStructure('course-js'); await api.studentCourseStructure('course-react'); await api.searchPosts({ query: 'lekcj' }); } },
