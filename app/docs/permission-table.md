...
Closed capability count: 84. Route rows: 185. Exported `Ctx` use-case rows: 160.
...
| api-key | `apps/server/src/internal-app.ts:733` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:735` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1177` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1177` | `(identity.staffRole \|\| identity.memberId)` |
