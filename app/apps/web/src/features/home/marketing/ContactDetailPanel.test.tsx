...
describe('contact detail', () => {
  it('edits names and tags without changing address identity and links import history', async () => {
    ...
    fireEvent.change(await screen.findByLabelText(en.directory.firstName), { target: { value: 'Anna' } });
    fireEvent.change(screen.getByLabelText(en.directory.tags), { target: { value: 'launch|news' } });
    ...
    expect(screen.getByText(en.directory.evidenceHint)).toBeInTheDocument();
