...
Closed capability count: 85. Route rows: 186. Exported `Ctx` use-case rows: 161.
...
| api-key | `apps/server/src/internal-app.ts:102` | `authenticateApiKey,` |
| api-key | `apps/server/src/internal-app.ts:736` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:738` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1180` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1180` | `(identity.staffRole \|\| identity.memberId)` |
