...
Emphasis is written as `_text_`, horizontal rules, strike-through and image references are preserved, and content the visual surface cannot represent — Markdown tables, task lists, raw HTML, and link or image destinations outside `https:`/`mailto:` (including root-relative paths and `{{template}}` variables) — keeps editing on the Markdown tab so nothing is dropped.
