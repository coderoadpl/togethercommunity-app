...
describe('direct message projections', () => {
  ...
  it('marks a conversation unread only for the recipient of the last message', () => {
    ...
    expect(
      toPublicDmConversation(conversation(), { userId: 'u2', lastReadAt: null }, otherParticipant).unread,
    ).toBe(false);
  });

  it('clears unread once the read cursor passes the last message', () => {
    expect(
      toPublicDmConversation(conversation(), { userId: 'u1', lastReadAt: LATER }, otherParticipant).unread,
    ).toBe(false);
    expect(
      toPublicDmConversation(conversation(), { userId: 'u1', lastReadAt: NOW }, otherParticipant).unread,
    ).toBe(true);
  });
  ...
  it('never marks a conversation without messages unread', () => {
    ...
    const projected = toPublicDmConversation(empty, { userId: 'u1', lastReadAt: null }, otherParticipant);
