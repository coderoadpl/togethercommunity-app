...
| api-key | `apps/server/src/internal-app.ts:919` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:921` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1365` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1365` | `(identity.staffRole \|\| identity.memberId)` |
...
| api-key | `apps/server/src/marketing-routes.ts:81` | `const apiIdentity = (tenant: Tenant): Identity => ({` |
| api-key | `apps/server/src/marketing-routes.ts:90` | `identity: apiIdentity(tenant),` |
| api-key | `apps/server/src/marketing-routes.ts:101` | `const key = headers.get(API_KEY_HEADER);` |
| api-key | `apps/server/src/marketing-routes.ts:103` | `const authenticated = await authenticateApiKey(resolved.value.tenant.id, key, deps);` |
| api-key | `apps/server/src/marketing-routes.ts:109` | `identity: apiIdentity(resolved.value.tenant),` |
| api-key | `apps/server/src/marketing-routes.ts:550` | `identity: apiIdentity({ id: settings.tenantId, slug: '', name: '', status: 'active', plan: 'self_hosted', contentVersion: 1 }),` |
