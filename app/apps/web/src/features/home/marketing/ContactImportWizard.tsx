...
import { Alert, Button, Checkbox, FormControlLabel, Step, StepLabel, Stepper, Table, TableBody, TableCell, TableHead, TableRow, Typography } from '@mui/material';
...
import { marketingImportMappingSchema, type MarketingContactImport, type MarketingImportValidation } from '#core/domain/index.js';
...
const ImportPreview = ({ preview }: { preview: MarketingImportValidation }) => {
  ...
  return <>
    ...
    {preview.errors.length ? <Alert severity="error"><Typography>{t.directory.errors}</Typography>{preview.errors.map((error, index) => <Typography key={index}>{t.directory.row} {error.rowNumber}: {error.message}</Typography>)}</Alert> : null}
  ...
  return <>
    ...
    <div ref={heading} tabIndex={-1}><SectionCard title={step === 0 ? t.directory.upload : step === 1 ? t.directory.mapping : t.directory.attestation}>
      {step === 0 ? <>
        ...
        <input type="file" accept=".csv,text/csv" aria-label={t.directory.file} disabled={pending} onChange={(event) => {
          ...
          setReading(true);
          void file.text().then((text) => { setCsv(text); setFileName(file.name); const result = parseMarketingImportCsv(text, delimiter || undefined); if (result.ok) { const mapped = mapMarketingImportCsv(result.value, { kind }); if (mapped.ok) setMapping(mapped.value.mapping); } }).catch(() => setFileError(t.directory.fileError)).finally(() => setReading(false));
        }} />
