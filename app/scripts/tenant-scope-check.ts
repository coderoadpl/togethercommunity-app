...
export const TENANT_SCOPE_EXCEPTIONS: Readonly<Record<string, string>> = {
  ...
  'ImpersonationSessionRepository.listLapsedTenantIds': 'A platform worker discovers tenants holding views that lapsed without an exit.',
...
export const NON_DATA_PORTS: Readonly<Record<string, string>> = {
  ...
  ImpersonationTokenCodec: 'Cryptographic primitive with no persistence access.',
  ...
  RealtimeNotifyDb: 'Realtime NOTIFY execution boundary; the tenant scope is the channel name.',
  ResendHttpClient: 'External Resend API transport boundary.',
  ResendHttpResponse: 'External Resend API response boundary.',
  ScopedSubscribers: 'In-memory listener registry with no persistence access.',
