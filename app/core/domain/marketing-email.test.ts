...
describe('U1 consent state derivation', () => {
  it('uses the latest row and applies double opt-in rules', () => {
    const rows = [
      consent('confirmed', '2026-07-02T00:00:00.000Z'),
      consent('withdrawn', '2026-07-03T00:00:00.000Z'),
      consent('granted', '2026-07-01T00:00:00.000Z'),
    ];
    expect(deriveConsentState(rows, definition(true))).toMatchObject({ state: 'withdrawn', active: false });
    expect(deriveConsentState([...rows, consent('granted', '2026-07-04T00:00:00.000Z')], definition(true)))
      .toMatchObject({ state: 'pending_confirmation', active: false });
    expect(deriveConsentState([consent('granted', '2026-07-01T00:00:00.000Z')], definition(false)))
      .toMatchObject({ state: 'active', active: true });
    expect(deriveConsentState([consent('confirmed', '2026-07-01T00:00:00.000Z')], definition(true)))
      .toMatchObject({ state: 'active', active: true });
...
describe('U3 suppression precedence', () => {
  it('makes an active suppression override confirmed consent and normalizes email for HMAC matching', () => {
    expect(deriveMarketingEligibility({ consent: { state: 'active', active: true, row: consent('confirmed', '2026-07-01T00:00:00.000Z') }, suppressed: true }))
  ...
  it('forbids complaint lifts and requires complete hard-bounce audit fields', () => {
    expect(liftSuppression(suppression('complaint'), { actorId: 'staff-1', liftedAt: '2026-07-02T00:00:00.000Z' }).ok)
      .toBe(false);
    expect(liftSuppression(suppression('hard_bounce'), { actorId: '', liftedAt: '2026-07-02T00:00:00.000Z' }).ok)
      .toBe(false);
    expect(liftSuppression(suppression('hard_bounce'), { actorId: 'staff-1', liftedAt: '2026-07-02T00:00:00.000Z' }))
      .toMatchObject({ ok: true, value: { liftedBy: 'staff-1', liftedAt: '2026-07-02T00:00:00.000Z' } });
  });
});
...
describe('U4 token semantics', () => {
  const token: UnsubscribeToken = {
    ...
    scope: 'consent:definition-1', createdAt: '2026-07-01T00:00:00.000Z', usedAt: null,
  };
  ...
  it('accepts opaque non-expiring tokens, resolves scope, and consumes idempotently', () => {
    ...
    const first = consumeUnsubscribeToken(token, '2026-07-02T00:00:00.000Z');
    ...
    expect(consumeUnsubscribeToken(first.value.token, '2036-07-02T00:00:00.000Z'))
      .toMatchObject({ ok: true, value: { newlyUsed: false } });
    expect(consumeUnsubscribeToken({ ...token, token: 'short' }, '2026-07-02T00:00:00.000Z').ok).toBe(false);
  });

  it('expires confirmation tokens at the boundary', () => {
    const confirmation = {
      id: 'confirmation-1', tenantId: 'tenant-1', token: '0123456789abcdef0123456789abcdef',
      marketingConsentRowId: 'consent-1', createdAt: '2026-07-01T00:00:00.000Z',
      expiresAt: '2026-07-02T00:00:00.000Z', usedAt: null,
    };
    expect(consentConfirmationTokenSchema.safeParse(confirmation).success).toBe(true);
    expect(confirmationTokenIsValid(confirmation, '2026-07-01T23:59:59.999Z')).toBe(true);
    expect(confirmationTokenIsValid(confirmation, confirmation.expiresAt)).toBe(false);
    expect(confirmationTokenIsValid({ ...confirmation, usedAt: '2026-07-01T12:00:00.000Z' }, '2026-07-01T13:00:00.000Z'))
      .toBe(false);
  });
});
