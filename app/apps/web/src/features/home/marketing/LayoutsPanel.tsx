  ...
  return (
    <PanelPage title={t.marketing.layoutsTitle} description={t.marketing.layoutsDescription} action={<Button component={Link} to="/panel/marketing/layouts/new" variant="contained">+ {t.common.add}</Button>}>
      <ListSection isEmpty={layouts.isSuccess && layouts.data.layouts.length === 0} empty={<StatusView state={{ kind: 'empty', title: t.marketing.layoutsEmpty, action: <Button component={Link} to="/panel/marketing/layouts/new">+ {t.common.add}</Button> }} />}>
        {layouts.isPending ? <StatusView state={{ kind: 'loading', label: t.marketing.layoutsLoading }} /> : layouts.isError ? <StatusView state={{ kind: 'error', message: localizePanelError(layouts.error, t), retry: { label: t.common.retry, onRetry: () => void layouts.refetch() } }} /> : (
          <Stack spacing="1rem">
...
export const LayoutDetailPage = () => {
  ...
  if (layouts.isError) return <PanelPage title={t.marketing.layoutsTitle} state={{ kind: 'error', message: localizePanelError(layouts.error, t), retry: { label: t.common.retry, onRetry: () => void layouts.refetch() } }} />;
