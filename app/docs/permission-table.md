...
Closed capability count: 110. Route rows: 299. Exported `Ctx` use-case rows: 243.
...
| api-key | `apps/server/src/internal-app.ts:1028` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:1030` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1498` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1498` | `(identity.staffRole \|\| identity.memberId)` |
