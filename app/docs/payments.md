...
A pre-existing unique index still constrains `product_grants` to one row per tenant, member and product regardless of mode. Granting the *other* mode for a member+product pair that already has a grant row therefore fails loudly (a database error), rather than silently, in both directions: a staff test purchase of a product the staff account already holds live, and — the direction that matters for real money — a live purchase, manual grant, subscription renewal or import for a member+product that a prior staff test purchase already granted. Testing a product the staff account does not already own live, in either mode, is unaffected. Lifting this requires a follow-up, owner-reviewed migration that drops the old index once no previous release still relies on it.
