  ...
  schedule: async (tenantId, input) => {
    try {
      return await db.transaction(async (tx) => {
        ...
        const snapshot = await createMarketingContactAudienceRepository(tx, deps).createSnapshot(tenantId, { campaignId: campaign.id, audience: input.audience, consentDefinitionId: campaign.consentDefinitionId, asOf: input.asOf });
        ...
        const updated = await repo.update(tenantId, { ...campaign, audience: input.audience, status: 'scheduled', sendAt: input.sendAt, audienceSnapshotId: snapshot.value.id, snapshotMaxContactId: snapshot.value.maxContactId, cursorContactId: null, candidateCount: snapshot.value.candidateCount, toSend: snapshot.value.eligibleCount, sent: 0, failed: 0, skipped: 0, errorCount: 0, pausedReason: null, audienceNameSnapshot: 'Contact lists', consentLabelSnapshot: version?.label ?? null });
