...
Closed capability count: 115. Route rows: 373. Exported `Ctx` use-case rows: 290.
...
| `GET /api/health/deep` | health:read | public | public | yes | public route manifest |
...
| `POST /api/m2m/subscriptions/adopt` | enrollment:create | api-key | api-key | yes | Tenant API key |
| `GET /api/m2m/subscriptions/stripe` | enrollment:create | api-key | api-key | yes | Tenant API key |
...
| `POST /api/subscriptions/adopt` | member:grant:write | owner, admin | owner, admin | yes | identity middleware + use-case guard |
| `GET /api/subscriptions/stripe` | member:commerce:read | owner, admin | owner, admin | yes | identity middleware + use-case guard |
...
| `stripe-subscription-adoption.ts#adoptStripeSubscription` | member:grant:write | owner, admin | owner, admin | yes | core/server/usecases/stripe-subscription-adoption.ts authorization call |
| `stripe-subscription-adoption.ts#listStripeSubscriptions` | member:commerce:read | owner, admin | owner, admin | yes | core/server/usecases/stripe-subscription-adoption.ts authorization call |
...
| `tenant-setup-readiness.ts#getTenantSetupReadiness` | tenant:onboarding:read | owner, admin | owner, admin | yes | core/server/usecases/tenant-setup-readiness.ts authorization call |
...
| api-key | `apps/server/src/internal-app.ts:1096` | `const authed = await authenticateApiKey(tenant.value.tenant.id, c.req.header(API_KEY_HEADER) ?? '', deps);` |
| api-key | `apps/server/src/internal-app.ts:1113` | `const authed = await authenticateApiKey(tenant.value.tenant.id, c.req.header(API_KEY_HEADER) ?? '', deps);` |
| staff-role | `apps/server/src/internal-app.ts:1623` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1623` | `(identity.staffRole \|\| identity.memberId)` |
...
| member-scope | `core/server/usecases/community.ts:236` | `if (tenantId !== null && identity.memberId !== null) {` |
| staff-role | `core/server/usecases/community.ts:250` | `if (ctx.identity.staffRole !== null) return false;` |
| staff-role | `core/server/usecases/community.ts:462` | `if (post.authorUserId !== actor.value.userId && !ctx.identity.staffRole) {` |
