...
describe('member event migration', () => {
  it('backfills existing member history into registry-valid events', async () => {
    ...
    await pool.query(
      `insert into member_subscriptions (id, tenant_id, member_id, product_id, price_id, provider, provider_subscription_id, status, current_period_end, created_at, updated_at) values ('subscription-history', 'tenant-history', 'member-history', 'product-history', 'price-history', 'stripe', 'sub_history', 'active', '2026-09-03T10:00:00.000Z', $1, $1)`,
      [NOW],
    );
    await pool.query(
