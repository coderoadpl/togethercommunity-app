...
describe('marketing list management', () => {
  it('loads counts only for the requested list', async () => {
    ...
    const buttons = await screen.findAllByRole('button', { name: pl.directory.loadCounts });
    expect(ids).toEqual([]);
    const button = buttons[0];
    if (!button) throw new Error('Missing counts button');
    await userEvent.click(button);
    await waitFor(() => expect(ids).toEqual([detail.list.id]));
    expect(screen.getAllByRole('button', { name: pl.directory.loadCounts })).toHaveLength(1);
  });
  it('uses revisions for updates and keeps key and kind immutable', async () => {
    ...
    expect(await screen.findByLabelText(pl.directory.key)).toBeDisabled();
    expect(screen.getByRole('combobox', { name: pl.directory.kind })).toHaveAttribute('aria-disabled', 'true');
    fireEvent.change(screen.getByLabelText(pl.directory.name), { target: { value: 'Renamed' } });
  ...
  it('shows live dynamic rules and counts without manual membership controls', async () => {
    ...
    expect(await screen.findByRole('combobox', { name: pl.directory.rule })).toHaveTextContent(pl.directory.tagRule);
    expect(screen.queryByRole('searchbox')).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: pl.directory.remove })).not.toBeInTheDocument();
    expect(await screen.findByText(new RegExp(`${pl.directory.contactCount}: 2`))).toBeInTheDocument();
  });
});
