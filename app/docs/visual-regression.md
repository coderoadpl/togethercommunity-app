...
device scale, and motion preference. It blocks non-local resources, waits for
the screen's explicit ready condition, network idle, and loaded fonts, then
freezes animations, transitions, and the caret before capture. Captures are
sequential and comparison has no retry or tolerance budget.
