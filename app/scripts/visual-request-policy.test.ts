...
describe('shared visual request policy', () => {
  ...
  it('returns the real client transport failure even if a fixture contains mutation success', async () => {
    const input = { courseId: 'course-js', lessonId: 'lesson-js-zmienne-1', moduleId: 'module-js-podstawy', chapterId: 'chapter-js-zmienne' };
    selectFixture({ scenario: 'transport-policy', principal: 'kursant.aktywny@together.dev', tenant: 'studio', route: '/', calls: {
      [fixtureKey('updateLastViewed', [input])]: { ok: true, value: {} },
      [fixtureKey('markSpaceSeen', [{ spaceId: 'space-studio-spolecznosc' }])]: { ok: true, value: {} },
    } });
    ...
    expect(await fixtureClient.markSpaceSeen({ spaceId: 'space-studio-spolecznosc' })).toMatchObject({ ok: false, error: { code: 'internal' } });
  });
});
