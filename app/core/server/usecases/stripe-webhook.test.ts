...
describe('fulfillStripeWebhook', () => {
  ...
  it('expires the grant at the paid period end when cancellation is scheduled', async () => {
    ...
    expect(Array.from(h.grants.values())[0]?.expiresAt).toBe('2026-08-20T10:00:00.000Z');
    ...
    expect(h.queued).toEqual([
      {
        ...
        payload: expect.objectContaining({
          ...
          accessEndsAt: '2026-08-20T10:00:00.000Z',
        }),
      },
    ]);
  });
  ...
  it.each([
    ['missing', null, now],
    ['expired', '2026-07-01T10:00:00.000Z', '2026-07-01T10:00:00.000Z'],
  ...
  it('cancels Stripe before locally canceling a refunded recurring subscription', async () => {
    ...
    const latePaid = await fulfillStripeWebhook(
      ...
      invoiceEvent({
        ...
        subscriptionId: 'sub-1',
        periodEnd: '2026-10-14T10:00:00.000Z',
      }),
      h.deps,
    );
