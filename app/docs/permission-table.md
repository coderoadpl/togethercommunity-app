...
| api-key | `apps/server/src/internal-app.ts:949` | `const presentedKey = c.req.header(API_KEY_HEADER);` |
| api-key | `apps/server/src/internal-app.ts:951` | `const authed = await authenticateApiKey(tenant.value.tenant.id, presentedKey, deps);` |
| staff-role | `apps/server/src/internal-app.ts:1395` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1395` | `(identity.staffRole \|\| identity.memberId)` |
...
| api-key | `apps/server/src/marketing-routes.ts:80` | `const apiIdentity = (tenant: Tenant): Identity => ({` |
| api-key | `apps/server/src/marketing-routes.ts:89` | `identity: apiIdentity(tenant),` |
| api-key | `apps/server/src/marketing-routes.ts:100` | `const key = headers.get(API_KEY_HEADER);` |
| api-key | `apps/server/src/marketing-routes.ts:102` | `const authenticated = await authenticateApiKey(resolved.value.tenant.id, key, deps);` |
| api-key | `apps/server/src/marketing-routes.ts:108` | `identity: apiIdentity(resolved.value.tenant),` |
| api-key | `apps/server/src/marketing-routes.ts:552` | `identity: apiIdentity({ id: settings.tenantId, slug: '', name: '', status: 'active', plan: 'self_hosted', contentVersion: 1 }),` |
