...
- **Output format:** a Markdown audit record using the fields required by the
  ...
  Scorecard SARIF artifact or workflow-run URL, CodeQL alerts or workflow-run
  URL, and separate repository findings from GitHub-configuration findings.
- **Standard anchor:**
...
- [`github/codeql-action` v3.37.4](https://github.com/github/codeql-action/releases/tag/v3.37.4) — `a2983b8bed1923f44751c5c43237f479442827b3`, providing `upload-sarif` in `scorecard.yml`
- [`github/codeql-action` v4.37.4](https://github.com/github/codeql-action/releases/tag/v4.37.4) — `f205ea1c3313d32999d8d6a48b4f6530d4437b38`, providing `init` and `analyze` in `codeql.yml`. The v4 scanning workflow deliberately leaves the existing Scorecard upload on v3 pending its separate migration.
...
3. Confirm CodeQL remains advisory rather than required.
4. Confirm forks and untrusted pull requests cannot reach privileged events,
   write tokens, deployment credentials, or repository-owner-only secrets.
5. Triage Scorecard 5.5.0 and CodeQL findings against Together's actual threat
   model and the existing [security posture](../security.md). Do not replace
   manual review with an aggregate score or unexamined scanner severity.
6. Inventory produced artifacts and provenance. If no distributable artifact
   has SLSA v1.2 Build provenance, retain the L0 baseline and file the gap rather
   than assigning a higher level.
7. Record settings that could not be observed as blind spots. The repository
