...
describe('marketing database repositories', () => {
  ...
  it('leaves a withdrawn marketing state when the deadline stops a consent-chain purge', async () => {
    ...
    await marketing.record(tenantId, evidence({
      id: 'chain-granted', status: 'granted', previousId: null, occurredAt: '2018-01-01T00:00:00.000Z',
    }));
    await marketing.record(tenantId, evidence({
      id: 'chain-confirmed', status: 'confirmed', previousId: 'chain-granted', occurredAt: '2019-01-01T00:00:00.000Z',
    }));
    await marketing.record(tenantId, evidence({
      id: 'chain-withdrawn', status: 'withdrawn', previousId: 'chain-confirmed', occurredAt: '2020-01-01T00:00:00.000Z',
    }));
    let monotonicRead = 0;
    const repository = createConsentEvidenceRetentionRepository(db);

    await expect(repository.purgeExpired(tenantId, '2021-01-01T00:00:00.000Z', {
      batchSize: 1,
      deadlineMs: 100,
