...
Closed capability count: 109. Route rows: 280. Exported `Ctx` use-case rows: 229.
...
| api-key | `apps/server/src/internal-app.ts:154` | `authenticateApiKey,` |
...
| staff-role | `apps/server/src/internal-app.ts:1414` | `(identity.staffRole \|\| identity.memberId)` |
| member-scope | `apps/server/src/internal-app.ts:1414` | `(identity.staffRole \|\| identity.memberId)` |
...
| api-key | `apps/server/src/marketing-routes.ts:94` | `identity: apiIdentity(tenant),` |
| api-key | `apps/server/src/marketing-routes.ts:105` | `const key = headers.get(API_KEY_HEADER);` |
| api-key | `apps/server/src/marketing-routes.ts:107` | `const authenticated = await authenticateApiKey(resolved.value.tenant.id, key, deps);` |
| api-key | `apps/server/src/marketing-routes.ts:113` | `identity: apiIdentity(resolved.value.tenant),` |
| api-key | `apps/server/src/marketing-routes.ts:640` | `identity: apiIdentity({ id: settings.tenantId, slug: '', name: '', status: 'active', plan: 'self_hosted', contentVersion: 1 }),` |
...
| member-scope | `core/server/usecases/community.ts:223` | `if (tenantId !== null && ctx.identity.memberId !== null) {` |
| staff-role | `core/server/usecases/community.ts:237` | `if (ctx.identity.staffRole !== null) return false;` |
| staff-role | `core/server/usecases/community.ts:432` | `if (post.authorUserId !== actor.value.userId && !ctx.identity.staffRole) {` |
