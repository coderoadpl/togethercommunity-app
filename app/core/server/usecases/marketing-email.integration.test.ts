...
describe('marketing e-mail use-case integration', () => {
  ...
  it('refreshes SES quota before the first API send and refuses a sandbox account', async () => {
    ...
    deps.quotaReader = {
      read: async () => ok({ ratePerSecond: 20, daily: 2000, sentLast24Hours: 25, inSandbox: true }),
    };
    const result = await sendMarketingMessages(ctx, [{
      to: 'member@example.test', memberId: null, campaignId: null, source: 'api',
    ...
    expect(await deps.sesSettings.findByTenant('tenant-1')).toMatchObject({
      quotaRatePerSec: 20, quotaDaily: 2000, quotaSentLast24Hours: 25,
  ...
  it('I5 permits one concurrent lease holder and allows stealing an expired lease', async () => {
    ...
    await deps.campaigns.update('tenant-1', campaign({ lockedUntil: '2026-07-22T09:59:59.000Z' }));
  ...
  it('I6 claims idempotency before execution, releases 4xx, retains 5xx, and sweeps TTL', async () => {
    ...
    expect(await repository.sweepExpired('2026-07-22T10:01:01.000Z')).toBe(1);
  });
  ...
  it('records only selected checkout consents with the current wording snapshot and DOI mail', async () => {
    ...
    const afterTokenExpiry = '2026-07-23T10:00:01.000Z';
  ...
  it('rejects future-dated consent evidence so a later withdrawal cannot be resurrected', async () => {
    ...
    const future = await recordMarketingConsent(ctx, {
      ...
      evidence: { collectedAt: '2027-01-01T00:00:00.000Z', proofRef: 'bad-clock' },
  ...
  it('scans all due campaign work and runs retention for every marketing tenant', async () => {
    ...
    expect(checkedBefore).toBe('2026-07-22T04:00:00.000Z');
  });
  ...
  it('schedules another tick when a campaign still has recipients after its budgeted batch', async () => {
    ...
    expect(deps.scheduler.scheduled).toEqual([{
      tenantId: 'tenant-1', campaignId: 'campaign-1', runAt: '2026-07-22T10:00:50.000Z',
    }]);
  });
  ...
  it('I11 does not consume a confirmation token when its pending consent row is temporarily unavailable', async () => {
    ...
    await deps.confirmations.create('tenant-1', { id: 'retry-token', tenantId: 'tenant-1', token, marketingConsentRowId: 'late-consent', createdAt: NOW, expiresAt: '2026-07-23T10:00:00.000Z', usedAt: null });
  ...
  it('supports suppression lifting, campaign CRUD gates, and retention orchestration', async () => {
    ...
    const scheduled = created.ok ? await scheduleCampaign(ctx, { campaignId: created.value.id, sendAt: '2026-07-23T10:00:00.000Z' }, deps) : created;
    ...
    expect(created.ok && (await scheduleCampaign(ctx, { campaignId: created.value.id, sendAt: '2026-07-24T10:00:00.000Z' }, deps)).ok).toBe(false);
