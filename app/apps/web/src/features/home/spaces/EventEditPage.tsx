...
const EventForm = ({
  ...
  const submittable = title.trim().length > 0 && timesFilled && !timeOrderBroken;

  const submit = (event: FormEvent) => {
    event.preventDefault();
    if (!submittable) return;
    onSubmit({ title: title.trim(), description, startsAt, endsAt, location, url });
  };
...
export const EventCreatePage = ({ spaceId }: { spaceId: string }) => {
  ...
  return (
    ...
    >
      <EventForm
        mode="create"
        initial={{ title: '', description: '', startsAt: '', endsAt: '', location: '', url: '' }}
