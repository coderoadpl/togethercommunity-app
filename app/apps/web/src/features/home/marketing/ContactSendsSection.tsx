...
import { deliveryStatusColor, deliveryStatusLabel, reasonLabel, sendStatusLabel } from './EmailSendSummary.js';
  ...
  return <SectionCard title={t.marketing.contactSends}>
    ...
    <Stack component="ul" useFlexGap spacing="0.5rem" sx={{ listStyle: 'none', m: 0, p: 0 }}>
      {sends.data?.sends.map((send) => {
        const reason = send.skipReason === null ? null : reasonLabel(send.skipReason, t);
        return (
          <Stack
            key={send.id}
            component="li"
