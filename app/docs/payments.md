...
A pre-existing unique index still constrains `product_grants` to one row per tenant, member and product regardless of mode, so a staff test purchase of a product the same staff account already holds live silently records no grant row (the order and any subscription still record normally). Testing a product the staff account does not already own live is unaffected. Lifting this requires a follow-up, owner-reviewed migration that drops the old index once no previous release still relies on it.
