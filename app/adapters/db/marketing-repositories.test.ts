...
import { campaignSends, consents, emailOutbox, marketingConsents, marketingOutbox, members, schedulerRuns, tenantSesSettings, tenants } from './schema.js';
...
describe('marketing database repositories', () => {
  ...
  it('projects campaign results in one tenant-scoped batch', async () => {
    ...
    await runs.start({
      ...
      finishedAt: null, durationMs: null, status: 'running', error: null,
      totals: { campaignsTouched: 0, sendsAttempted: 0, sent: 0, failed: 0, skipped: 0, reEnqueued: false },
      createdAt: NOW,
    });
    await runs.finalize('run-results', {
      finishedAt: '1998-07-22T00:00:01.000Z', durationMs: 1000, status: 'completed', error: null,
